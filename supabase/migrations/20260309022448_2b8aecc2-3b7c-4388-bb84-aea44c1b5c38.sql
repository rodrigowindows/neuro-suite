...
-- Drop and recreate every policy across all tables
...
CREATE POLICY "Managers can view all checkins" ON public.daily_checkins FOR SELECT USING (has_role(auth.uid(), 'manager'::app_role) OR has_role(auth.uid(), 'admin'::app_role));
...
-- ===== nr1_conversations =====
DROP POLICY IF EXISTS "Users can view own nr1 conversations" ON public.nr1_conversations;
DROP POLICY IF EXISTS "Users can insert own nr1 conversations" ON public.nr1_conversations;
DROP POLICY IF EXISTS "Users can update own nr1 conversations" ON public.nr1_conversations;

CREATE POLICY "Users can view own nr1 conversations" ON public.nr1_conversations FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own nr1 conversations" ON public.nr1_conversations FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own nr1 conversations" ON public.nr1_conversations FOR UPDATE USING (auth.uid() = user_id);
